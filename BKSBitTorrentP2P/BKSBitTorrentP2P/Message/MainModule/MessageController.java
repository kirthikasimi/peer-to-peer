...
public class MessageController {
	...
	public synchronized int getMessageLength(Type messageType, int pieceIndex) {
		...
		case PIECE:
			System.out.println("Shared file" + dataController.getPiece(pieceIndex) + " asking for piece " + pieceIndex);
			int payloadLength = 5 + FileHandler.getInstance().getPiece(pieceIndex).length;
	...
	public synchronized byte[] getMessagePayload(Type messageType, int pieceIndex) {
		...
		case REQUEST:
			...
			break;
		case PIECE:
			byte[] piece = dataController.getPiece(pieceIndex);
